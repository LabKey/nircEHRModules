 ...
 */
Ext4.define('NIRC_EHR.form.field.PlanTextArea', {
    extend: 'Ext.form.field.TextArea',
    alias: 'widget.nirc_ehr-plantextarea',
    ...
    onRender : function(ct, position){
        ...
        if (panel){
            ...
            LDK.Utils.logToServer({
                message: 'Unable to find nirc_ehr-formpanel in PlanTextArea'
            })
        }

        var dataEntryPanel = this.up('nirc_ehr-dataentrypanel');
        if (dataEntryPanel){
            this.mon(dataEntryPanel, 'animalchange', this.onAnimalChange, this, {buffer: 100});
        }
        else {
            LDK.Utils.logToServer({
                message: 'Unable to find nirc_ehr-dataentrypanel in PlanTextArea'
            })
        }
    ...
    getMostRecentP2: function(rec, cb, alwaysUseCallback){
        ...
        LABKEY.Query.executeSql({
            schemaName: 'study',
            sql: 'SELECT c.Id, c.p2 as mostRecentP2, c.caseid, c.caseid.category as caseCategory, (CASE WHEN c.caseid.enddate = NULL THEN true ELSE false END) as isActive FROM study.clinRemarks c WHERE (c.category != \'Replaced SOAP\' OR c.category IS NULL) AND c.p2 IS NOT NULL AND c.Id = \'' + rec.get('Id') + '\' ORDER BY c.date DESC LIMIT 1',
